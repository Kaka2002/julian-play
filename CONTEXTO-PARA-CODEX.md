...
## Preservação do robô comercial após resposta da gestão na versão 1.3.57

- Mensagens enviadas pelo Assistente de gestão e reenviadas pelo WhatsApp Web sem ID completo são reconhecidas pelo texto recentemente registrado pelo robô. Elas não são classificadas como atendimento humano e não pausam a conversa comercial.
...
## Modelos alternativos da campanha de indicação na versão 1.3.63

- A instalação passa a ter dois modelos de campanha: **Amizade que vale presente**, com a mensagem anterior, e **Indique e ganhe 3 meses**, com a regra aprovada de dois novos clientes, três meses ativos de cada um, benefício após conferência e sem acúmulo.
...
- A versão de entrega atual é **1.3.42**. A tela `Financeiro > Rendimentos` possui a ação `Sincronizar Mercado Pago`, que consulta o relatório oficial de liberações já gerado na conta e importa somente movimentos identificados pelo Mercado Pago como `asset_management_gain`. Cada operação externa é registrada uma única vez; PIX, saques, estornos e rendimentos negativos não são importados. A migração `2026-09-23-015-importacoes-rendimentos-mercado-pago` guarda somente identificadores de deduplicação e vínculos locais. É necessário configurar o relatório de liberações na conta Mercado Pago e manter o Access Token já usado pela integração PIX.

- A versão de entrega atual é **1.3.41**. O Financeiro possui a tela **Rendimentos**, acessível pelo botão de mesmo nome, para lançar rendimentos da conta que recebe os pagamentos dos clientes. O saldo mensal soma pagamentos válidos de clientes e rendimentos válidos, depois subtrai despesas válidas; os rendimentos ficam separados dos pagamentos de planos e podem ser editados ou removidos do resumo com preservação do histórico. A migração `2026-09-23-014-rendimentos-financeiros` cria a tabela isolada e o executor gera o backup pré-migração. Afeta painel administrador, comerciais no servidor e instalações locais; Painel Mestre inalterado. Não há integração automática com instituições financeiras nesta versão e nenhum acesso bancário é armazenado.
...
- Para não classificar contratos trimestrais, semestrais ou anuais como inadimplência nos meses posteriores ao pagamento, esses planos compõem somente a receita recorrente mensal equivalente enquanto estiverem ativos. O indicador **Vendas antecipadas no mês** mostra somente pagamentos desses planos realizados no mês filtrado, sem somar contratos antigos como caixa atual. Em `Financeiro > Despesas`, a comparação é apresentada como **Variação de caixa**; não usa o rótulo de valor a receber. Afeta painel administrador, clientes comerciais no servidor e instalações locais, sem mudar contratos, pagamentos ou saldo dos clientes.
...
- Uma renovação trimestral, semestral ou anual recente compõe a venda antecipada do mês. Contratos pagos em períodos anteriores não são somados novamente, pois sua contribuição permanece somente na receita recorrente mensal equivalente.
...
- A privacidade de valores monetários usa um botão de olho ao lado de “Receita
  Mensal Recorrente” e no cabeçalho da tela Financeiro; na página de pagamentos
  manuais, o botão fica no cabeçalho da própria tela. Por padrão, os valores visíveis são mascarados
