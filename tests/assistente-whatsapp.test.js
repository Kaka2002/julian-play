...
test('assistente inicia desligado e só aceita números autorizados', () => {
    const r = executarIsolado(`(async()=>{
        ...
        const permitido=await a.responderConsulta({texto:'ajuda',telefone:'5511999999999@c.us'});
        const negado=await a.responderConsulta({texto:'ajuda',telefone:'5511888888888'});
        process.stdout.write(JSON.stringify({inicial,permitido,negado})); db.close();
    })().catch(e=>{console.error(e);process.exit(1)})`);
    try {
        const resultado = JSON.parse(r.stdout);
        assert.equal(resultado.inicial, null);
        assert.match(resultado.permitido, /ASSISTENTE DE GESTÃO/);
...
test('assistente responde resumo e consulta sem alterar cliente ou pagamento', () => {
    const r = executarIsolado(`(async()=>{
        ...
        const resumo=await a.responderConsulta({texto:'resumo do mês',telefone:'5511999999999'});
        const cliente=await a.responderConsulta({texto:'consultar cliente Ana',telefone:'5511999999999'});
        const depois=await new Promise((ok,no)=>db.get('SELECT COUNT(*) total FROM cliente_pagamentos',[],(e,x)=>e?no(e):ok(x.total)));
        process.stdout.write(JSON.stringify({resumo,cliente,antes,depois})); db.close();
    })().catch(e=>{console.error(e);process.exit(1)})`);
    try {
        const resultado = JSON.parse(r.stdout);
