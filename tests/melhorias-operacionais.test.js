...
test('ficha do cliente oferece cópia segura para campos textuais e URLs de acesso', () => {
    const fs = require('fs');
    const rota = fs.readFileSync(path.join(__dirname, '..', 'routes', 'clientesRoute.js'), 'utf8');
    assert.match(rota, /function ativarCopiaDosCampos/);
...
test('painel separa receita recorrente da receita realmente recebida no mês', () => {
    ...
    const analise = contexto.calcularAnaliseReceitaMensal({ total: 35 }, [{ plano: 'Trimestral', valorPlano: '96,00', valorTotal: '96,00', diasContrato: 90 }], [{ nome: 'Mensal', valor: '35,00' }]);
    ...
    assert.equal(analise.vendasAntecipadas.total, 96);
    assert.equal(analise.vendasAntecipadas.quantidade, 1);
    assert.equal(analise.vendasAntecipadas.total, 96);
    assert.equal(Math.round(analise.vendasAntecipadas.percentualDesconto), 9);
    assert.match(fonte, /Receita recebida neste mês/);
    assert.match(fonte, /Vendas antecipadas no mês/);
