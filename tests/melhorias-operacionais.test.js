...
test('layout oferece botao de voltar ao topo somente em paginas longas e roladas', () => {
    ...
    assert.match(rota, /window\.scrollY > 80/);
...
test('paginacoes permitem escolher quantidade com padrao inicial de cinco', () => {
    const rota = fs.readFileSync(path.join(repoRoot, 'routes', 'clientesRoute.js'), 'utf8');
    assert.match(rota, /const OPCOES_POR_PAGINA = \[5, 10, 20, 40, 60, 80, 100\]/);
    assert.match(rota, /function quantidadePorPagina\(valor, padrao = 5\)/);
    ...
    assert.match(rota, /const FINANCEIRO_POR_PAGINA = 5/);
    assert.match(rota, /const DASHBOARD_VENCIMENTOS_POR_PAGINA = 5/);
});
...
test('painel separa receita recorrente da receita realmente recebida no mês', () => {
    ...
    const analise = contexto.calcularAnaliseReceitaMensal({ total: 35 }, [{ plano: 'Trimestral', valorPlano: '96,00', valorTotal: '96,00', diasContrato: 90 }], [{ nome: 'Mensal', valor: '35,00' }], [{ pagamentoPlano: 'Anual', pagamentoValorPlano: '336,00', pagamentoDiasContrato: 365 }]);
    assert.equal(recorrente.total, 35);
    assert.equal(real.total, 45);
    assert.equal(real.pagamentos, 1);
    assert.equal(analise.vendasAntecipadas.quantidade, 1);
    assert.equal(analise.vendasAntecipadas.total, 336);
    assert.equal(analise.vendasAntecipadas.novasNoMes.quantidade, 1);
    assert.equal(analise.vendasAntecipadas.novasNoMes.total, 96);
    assert.equal(Math.round(analise.vendasAntecipadas.percentualDesconto), 20);
    assert.match(fonte, /Receita recebida neste mês/);
    assert.match(fonte, /Contratos antecipados em vigência/);
    assert.match(fonte, /Trimestral.*Semestral.*Anual/);
    assert.doesNotMatch(fonte.slice(fonte.indexOf('function receitaMensalCard('), fonte.indexOf('function cardVencimento(')), /variação não é tratada como valor em atraso/);
    assert.match(fonte, /Variação de caixa/);
