...
test('artefato externo de entrega tem nome fixo e nao acumula ZIPs',()=>{
 ...
 assert.match(gerador,/ENVIAR_AO_CLIENTE\.zip/);
 assert.match(gerador,/Get-FileHash[^\n]+SHA256/);
 assert.match(gerador,/JULIAN-PLAY-\*\.zip/);
 assert.match(gerador,/ENVIAR_AO_CLIENTE-v\*\.zip/);
 assert.doesNotMatch(gerador,/Select-Object -Skip \$Retencao/);
});
...
test('deploy prepara e testa a versao antes da parada e possui rollback automatico',()=>{
 ...
 assert.match(atualizar,/\$estadoPrincipal\.porta = \$portaInstalacao/);
