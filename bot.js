...
function getSaudacao() {
    const hora = new Date().getHours() - 3; // Ajuste manual simples para fuso horário de Brasília no Render
...
function isPalavraChave(texto) {
    ...
    const palavras = ['menu', 'dia', 'tarde', 'noite', 'oi', 'olá', 'ola', 'teste'];
...
async function processarResposta(from, texto) {
    ...
    if (estado === 'menu_principal') {
        switch(texto) {
            case '1': usuario.estado = 'planos'; usuarios.set(from, usuario); await menuPlanos(from); break;
            case '2': usuario.estado = 'dispositivos'; usuarios.set(from, usuario); await menuDispositivos(from); break;
            case '3': usuario.estado = 'renovacao_planos'; usuarios.set(from, usuario); await menuRenovacao(from); break;
            case '4': usuario.estado = 'ativacao'; usuarios.set(from, usuario); await menuAtivacao(from); break;
            case '0': await finalizarAtendimento(from, nome); break;
            default: await opcaoInvalida(from); await menuPrincipal(from, nome);
        }
        return;
    }

    if (estado === 'planos') {
        if (texto === '9') { usuario.estado = 'dispositivos'; usuarios.set(from, usuario); await menuDispositivos(from); }
        else if (texto === '10') { usuario.estado = 'menu_principal'; usuarios.set(from, usuario); await menuPrincipal(from, nome); }
        else { await opcaoInvalida(from); await menuPlanos(from); }
        return;
    }

    if (estado === 'dispositivos') {
        if (texto === '0') { usuario.estado = 'menu_principal'; usuarios.set(from, usuario); await menuPrincipal(from, nome); }
        else if (texto === '3') { usuario.dispositivo = 'Smart TV'; usuario.estado = 'marcas_smart_tv'; usuarios.set(from, usuario); await menuMarcasSmartTV(from); }
        else if (['1', '2', '4', '5', '6', '7', '8'].includes(texto)) {
            const dispositivosMap = { '1': 'TV Box', '2': 'iPhone', '4': 'Android TV', '5': 'Computador', '6': 'Mi Stick TV', '7': 'Fire Stick TV', '8': 'Smartphone Android' };
            let msgConfirmacao = `✅ *TESTE GRÁTIS SOLICITADO COM SUCESSO!*\n\n*Resumo do pedido:*\n📱 Dispositivo: ${dispositivosMap[texto]}\n\nUm atendente entrará em contato em breve com as instruções.\n\nDigite *0* para voltar ao Menu Principal`;
            await enviarMensagem(from, msgConfirmacao);
            usuario.estado = 'menu_principal';
            usuarios.set(from, usuario);
        } else { await opcaoInvalida(from); await menuDispositivos(from); }
        return;
    }
    ...
    if (estado === 'marcas_smart_tv') {
        if (texto === '0') { usuario.estado = 'menu_principal'; usuarios.set(from, usuario); await menuPrincipal(from, nome); }
            ...
            const marcasMap = { '1': 'TV LG', '2': 'TV PHILCO', '3': 'TV PHILIPS', '4': 'TV ROKU', '5': 'TV SAMSUNG', '6': 'ANDROID TV', '7': 'TV TCL', '8': 'TV PANASONIC', '9': 'TV XIAOMI' };
            let msgConfirmacao = `✅ *TESTE GRÁTIS SOLICITADO COM SUCESSO!*\n\n*Resumo do pedido:*\n📱 Dispositivo: Smart TV\n📺 Marca: ${marcasMap[texto]}\n\nUm atendente entrará em contato em breve.\n\nDigite *0* para voltar ao Menu Principal`;
            await enviarMensagem(from, msgConfirmacao);
            usuario.estado = 'menu_principal';
            usuarios.set(from, usuario);
        } else { await opcaoInvalida(from); await menuMarcasSmartTV(from); }
        return;
    }

    if (estado === 'renovacao_planos') {
        if (texto === '0') { usuario.estado = 'menu_principal'; usuarios.set(from, usuario); await menuPrincipal(from, nome); }
        else if (texto === '1') { await enviarMensagem(from, '💰 *GERANDO QR CODE PIX PARA PLANO MENSAL - R$ 35,00*\n\nAguarde...'); await enviarQRCodePIX(from, '00020126330014br.gov.bcb.pix011161319147704520400005303986540535.005802BR5924Carlos Henrique Julianel6009Sao Paulo62230519daqr2188072805101086304D6FF', '35,00', 'PLANO MENSAL'); usuario.estado = 'renovacao_aguardando_comprovante'; usuarios.set(from, usuario); }
        else if (texto === '2') { await enviarMensagem(from, '💰 *GERANDO QR CODE PIX PARA PLANO TRIMESTRAL - R$ 80,00*\n\nAguarde...'); await enviarQRCodePIX(from, '00020126330014br.gov.bcb.pix011161319147704520400005303986540580.005802BR5924Carlos Henrique Julianel6009Sao Paulo62230519daqr21880728073011363046BA4', '80,00', 'PLANO TRIMESTRAL'); usuario.estado = 'renovacao_aguardando_comprovante'; usuarios.set(from, usuario); }
        else if (texto === '3') { await enviarMensagem(from, '💰 *GERANDO QR CODE PIX PARA PLANO SEMESTRAL - R$ 150,00*\n\nAguarde...'); await enviarQRCodePIX(from, '00020126330014br.gov.bcb.pix0111613191477045204000053039865406150.005802BR5924Carlos Henrique Julianel6009Sao Paulo62230519daqr2188072808042136304F414', '150,00', 'PLANO SEMESTRAL'); usuario.estado = 'renovacao_aguardando_comprovante'; usuarios.set(from, usuario); }
        else if (texto === '4') { await enviarMensagem(from, '💰 *GERANDO QR CODE PIX PARA PLANO ANUAL - R$ 275,00*\n\nAguarde...'); await enviarQRCodePIX(from, '00020126330014br.gov.bcb.pix0111613191477045204000053039865406275.005802BR5924Carlos Henrique Julianel6009Sao Paulo62230519daqr21880728085532063048B42', '275,00', 'PLANO ANUAL'); usuario.estado = 'renovacao_aguardando_comprovante'; usuarios.set(from, usuario); }
        else { await opcaoInvalida(from); await menuRenovacao(from); }
        return;
    }
    ...
    if (estado === 'ativacao') {
        if (texto === '0') { usuario.estado = 'menu_principal'; usuarios.set(from, usuario); await menuPrincipal(from, nome); }
        else { await opcaoInvalida(from); await menuAtivacao(from); }
        return;
    }
}
client.on('message', async (message) => {
    ...
    if (isPalavraChave(body)) {
        ...
        if (!usuario) {
            usuarios.set(from, { nome: null, estado: 'aguardando_nome' });
