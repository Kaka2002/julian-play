...
function criarMensagensInformativasRoute({ getClient, listarClientes, normalizarTelefone, dataDir, layout } = {}) {
    ...
    router.get('/', async (req, res) => {
        ...
        const paginaHistorico = Math.max(1, Number.parseInt(req.query.paginaHistorico, 10) || 1);
        const porPaginaHistorico = 5;
        const totalPaginasHistorico = Math.max(1, Math.ceil(historico.length / porPaginaHistorico));
        const paginaAtualHistorico = Math.min(paginaHistorico, totalPaginasHistorico);
        const inicioHistorico = (paginaAtualHistorico - 1) * porPaginaHistorico;
        const itensHistorico = historico.slice(inicioHistorico, inicioHistorico + porPaginaHistorico);
        const resumo = historico.length ? `<section class="panel informativo-historico"><style>.informativo-historico{font-size:15px;max-width:1600px;margin-left:auto;margin-right:auto}.informativo-historico h2{margin-top:0}.informativo-historico article{padding:12px 0;border-top:1px solid #e5eaf2;display:grid;gap:6px}.informativo-historico small{color:#667085}.informativo-historico .pagination{margin-top:12px}</style><h2>Últimos envios</h2>${itensHistorico.map(item => `<article><strong>${escapar(item.data)}</strong><div>Destinatários: ${escapar(item.clientes.join(', ') || 'Nenhum')}</div><div>${escapar(item.texto || 'Sem texto')}</div><small>${item.imagens.length} imagem(ns): ${escapar(item.imagens.join(', '))}</small></article>`).join('')}<div class="pagination"><span>Página ${paginaAtualHistorico} de ${totalPaginasHistorico}</span>${paginaAtualHistorico > 1 ? `<a class="button secondary" href="/mensagens-informativas?paginaHistorico=${paginaAtualHistorico - 1}">Anterior</a>` : ''}${paginaAtualHistorico < totalPaginasHistorico ? `<a class="button secondary" href="/mensagens-informativas?paginaHistorico=${paginaAtualHistorico + 1}">Próxima</a>` : ''}</div></section>` : '';
