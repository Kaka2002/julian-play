...
const { importarRendimentosMercadoPago } = require('../services/mercadoPagoService');
const { obterConciliacaoSaldo, salvarConciliacaoSaldo } = require('../services/conciliacaoSaldoFinanceiroService');
...
function calcularAnaliseReceitaMensal(receitaRecorrente = {}, pagamentos = [], planos = []) {
    ...
    const totalAntecipado = antecipados.reduce((soma, pagamento) => soma + numeroMoeda(pagamento.valorPlano || pagamento.valorTotal), 0);
...
<body class="${escapar(bodyClass)}">
    ...
    <main>
        ...
        ${licenca.cliente && (ativo === 'painel' || ativo === 'manutencao') ?`<div style="margin:10px 0;padding:8px 14px;border:1px solid #cfe0f5;border-radius:8px;background:#f7fbff;color:#365477;font-size:12px">Licenciado para: <strong>${escapar(licenca.cliente)}</strong> · ID: ${escapar(config.instalacaoId || 'não informado')}</div>` : ''}
        ${conteudo}
    </main>
...
function receitaMensalCard(receita, receitaReal, planos = []) {
    ...
    const notaAntecipada = analise.vendasAntecipadas.quantidade
        ?`${analise.vendasAntecipadas.quantidade} venda(s): ${analise.vendasAntecipadas.porPlano.map(item => `${item.nome} (${item.quantidade}, ${item.descontoPercentual.toFixed(1).replace('.', ',')}% de desconto)`).join(' | ')}. Economia total: ${formatarMoeda(analise.vendasAntecipadas.descontoTotal)}.`
    ...
    <section class="panel dashboard-campaign" style="margin-bottom:24px;">
        <div class="panel-head">
            ...
            <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap; justify-content:flex-end;">
                ...
                <a class="button secondary" href="/indicacoes">Controlar indicações</a>
                <form method="post" action="/clientes/disparar-amizade-presente-cliente" onsubmit="return confirm('Enviar a campanha somente para o cliente selecionado?');" style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                    <select name="clienteId" required style="min-width:220px;">
...
function telaRendimentosFinanceiros({ rendimentos = [], filtros = {}, receitaMes = 0, despesasMes = 0, conciliacao = {} }) {
    const resumo = resumoRendimentosFinanceiros(rendimentos);
    const saldo = receitaMes + resumo.total - despesasMes;
    const saldoInicial = numeroMoeda(conciliacao.saldoInicial);
    const saldoBanco = numeroMoeda(conciliacao.saldoBanco);
    const saldoEsperado = saldoInicial + saldo;
    const diferencaConciliacao = saldoBanco - saldoEsperado;
    ...
    return `<section class="page-title page-title-with-action"><div><h1>Rendimentos</h1><div class="subtitle">Rendimentos da conta somados ao saldo real do mês, separados dos pagamentos dos clientes.</div></div><a class="button secondary" href="/financeiro">Voltar ao financeiro</a></section><section class="clients-panel" style="margin-bottom:20px;"><div class="panel-head"><div><h2 class="panel-title">Conciliação do saldo bancário</h2><div class="subtitle">Saldo inicial + créditos + rendimentos − despesas.</div></div></div><form class="form-grid" method="post" action="/financeiro/rendimentos/conciliacao-saldo"><input type="hidden" name="mes" value="${escapar(filtros.mes || mesAtualInput())}"><label>Saldo inicial<input name="saldoInicial" inputmode="decimal" value="${escapar(conciliacao.saldoInicial || '0,00')}"></label><label>Saldo atual no Mercado Pago<input name="saldoBanco" inputmode="decimal" value="${escapar(conciliacao.saldoBanco || '0,00')}"></label><div class="actions"><button class="button secondary" type="submit">Salvar conciliação</button></div></form><div class="notice" style="margin-top:12px;">Esperado: <strong>${escapar(formatarMoeda(saldoEsperado))}</strong> · Banco: <strong>${escapar(formatarMoeda(saldoBanco))}</strong> · Diferença: <strong>${escapar(formatarMoeda(diferencaConciliacao))}</strong></div></section><div class="actions" style="margin:0 0 20px;"><form method="post" action="/financeiro/rendimentos/sincronizar-mercado-pago"><input type="hidden" name="mes" value="${escapar(filtros.mes || mesAtualInput())}"><button class="button secondary" type="submit">${icon('trend')} Sincronizar Mercado Pago</button></form></div><section class="metrics">${metricCard({ label: 'Rendimentos recebidos', valor: formatarMoeda(resumo.total), nota: `${resumo.quantidade} lançamento(s) no mês`, tipo: 'green', icone: 'trend' })}${metricCard({ label: 'Receita de clientes', valor: formatarMoeda(receitaMes), nota: 'Pagamentos válidos no mês', tipo: 'info', icone: 'financeiro' })}${metricCard({ label: 'Despesas pagas', valor: formatarMoeda(despesasMes), nota: 'Lançamentos válidos no mês', tipo: 'red', icone: 'trash' })}${metricCard({ label: 'Saldo do mês', valor: formatarMoeda(saldo), nota: 'Clientes + rendimentos - despesas', tipo: saldo >= 0 ?'info' :'orange', icone: 'financeiro' })}</section><form class="clients-toolbar" method="get" action="/financeiro/rendimentos"><input type="month" name="mes" value="${escapar(filtros.mes || mesAtualInput())}" onchange="this.form.submit()"><div class="clients-search">${icon('search')}<input name="busca" value="${escapar(filtros.busca || '')}" placeholder="Buscar descrição, instituição ou observação..."></div><select name="status" onchange="this.form.submit()">${[['validos','Válidos'],['removidos','Removidos'],['todos','Todos']].map(([valor,texto]) => `<option value="${valor}" ${valor === filtros.status ?'selected' :''}>${texto}</option>`).join('')}</select><button class="button secondary" type="submit">${icon('search')} Filtrar</button></form>${formularioRendimentoFinanceiro({ mes: filtros.mes || mesAtualInput() })}<section class="clients-panel"><table class="clients-table"><thead><tr><th>Data</th><th>Descrição</th><th>Instituição</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody>${linhas}</tbody></table></section>`;
}
...
function telaManutencao(status = {}, opcoes = {}) {
    ...
    ${manutencaoRestrita ?'' : `<section class="panel" style="margin-bottom:24px;">
        ...
        <form class="fields" method="post" action="/manutencao/robo" style="padding-top:0;">
            ...
            <div class="notice full"><strong>Comandos e perguntas da gestão (somente números autorizados):</strong><br>• <strong>o que preciso resolver hoje?</strong> mostra recebimentos, comprovantes, vencidos e vencimentos próximos · <strong>resumo do mês</strong> ou <strong>quanto recebi este mês?</strong> · <strong>como foi a semana?</strong> traz recebimentos, despesas, saldo, comprovantes e vencimentos da semana atual.<br>• <strong>vencimentos hoje</strong> ou <strong>quais clientes vencem hoje?</strong> · <strong>clientes vencidos</strong> · <strong>quem vence nos próximos 3 dias?</strong> · <strong>quanto tenho para receber?</strong><br>• <strong>consultar cliente Nome ou telefone</strong> ou <strong>situação do cliente Nome</strong>. Depois pergunte <strong>ele está em dia?</strong> ou <strong>quando vence?</strong><br>• <strong>comprovantes pendentes</strong>. Para confirmar após conferir no banco: <strong>confirmar comprovante 123</strong>, depois <strong>CONFIRMAR 123</strong>.<br>• <strong>menu</strong> continua abrindo o robô comercial; use <strong>menu gestão</strong> para ver a ajuda administrativa.</div>
            <div class="actions full">
    ...
    <section class="panel">
        ...
        ${manutencaoRestrita ? '' : `<div class="notice" style="margin:0 20px 18px;">
            <strong>Retenção operacional:</strong> ${escapar(status.retencaoEventos?.totalRemovivel || 0)} evento(s) operacional(is) com mais de 365 dias podem ser removidos agora. Eventos financeiros, segurança, privacidade, exclusões, reclamações e auditoria são preservados permanentemente.
        </div>
...
async function renderizarPaginaIndicacoes(req, res) {
    ...
    await renderizar(res, {
        ...
        conteudo: telaIndicacoes({ clientes, indicacoes }),
        mensagem: req.query.mensagem || '',
        ativo: 'campanhas'
    });
}

router.get('/indicacoes', renderizarPaginaIndicacoes);

router.post('/indicacoes', async (req, res) => {
    try {
        const indicacao = await registrarIndicacao(req.body);
        await adicionarNotaCliente(indicacao.indicador.id, `Programa de indicação: ${indicacao.indicado.nome} foi registrado como indicado.`);
        await adicionarNotaCliente(indicacao.indicado.id, `Programa de indicação: cadastro vinculado a ${indicacao.indicador.nome}.`);
...
router.post('/indicacoes/:id/cancelar', async (req, res) => {
    try {
        await cancelarIndicacao(req.params.id, req.body.motivo);
        return res.redirect('/indicacoes?mensagem=Indicação cancelada e preservada no histórico.');
    } catch (err) {
        return res.redirect(`/indicacoes?mensagem=${encodeURIComponent(err.message || 'Não foi possível cancelar a indicação.')}`);
    }
});
...
router.use(criarAtendimentosRoute({
    desativarCache,
    listarAtendimentos,
    listarClientes,
    resumoAtendimentos,
    ...
    mensagemAtendimentoPadrao
}));

router.use(criarCrmRoute({
    desativarCache,
    listarLeads,
    listarClientes,
    listarPlanosComerciais,
...
router.use(criarFinanceiroRoute({
    ...
    obterConciliacaoSaldo,
    salvarConciliacaoSaldo,
